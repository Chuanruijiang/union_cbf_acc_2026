...
def system_dynamics(x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    f = np.array([sym.Polynomial(-x[1]), sym.Polynomial(), sym.Polynomial()])
    g = np.array(
        [
...
def original_to_extended_state_space(input_points: np.ndarray) -> np.ndarray:
